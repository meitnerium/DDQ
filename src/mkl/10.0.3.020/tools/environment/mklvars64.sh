...
then
    export LD_LIBRARY_PATH="${MKLROOT}/lib/64"
else
    export LD_LIBRARY_PATH="${MKLROOT}/lib/64:$LD_LIBRARY_PATH"
fi
...
if [ -z "${LIBRARY_PATH}" ]
then
    export LIBRARY_PATH="${MKLROOT}/lib/64"
else
    export LIBRARY_PATH="${MKLROOT}/lib/64:$LIBRARY_PATH"
fi
