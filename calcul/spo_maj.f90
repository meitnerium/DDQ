...
!                derg=dsin(wir*t/(2*nc))*dcos(wir*t/(2*nc))
                champ=E0*dcos(wir*t+phase)!(e*derg+g*dere)
!        else
!                champ=0.d0
!        end if
        return
...
! changer l init de la fft au debut du prog et le stopper a la fin
contains  
...
complex*16,dimension(:),allocatable :: ei
	n=2 !Dimensions
	allocate(a(n,n),b(n,n),ei(n))
	pi=3.14159265358979323846264338327950288419716939937510d0!MATH_PI
 	delk=2.d0*pi/(npos*delr)
 do ll = 1, npos
...
!Diag as MEDYS
	a(1,2)=xmu12(ll)*champ
	a(2,1)=xmu12(ll)*champ
	a(1,1)=pot(1,ll)
	a(2,2)=pot(2,ll)
	call Diagonalize(a,n,b,ei)
	vp1=ei(1)
	vp2=ei(2)
	cwtemp=(cw1(ll)*b(2,2)+cw2(ll)*(-b(1,2)))*(b(1,1)*b(2,2)-b(1,2)*b(2,1))
	cw2(ll)=(cw2(ll)*b(1,1)+cw1(ll)*(-b(1,2)))*(b(1,1)*b(2,2)-b(1,2)*b(2,1))
	cw1(ll)=cwtemp
	cphase1 = cdexp(-cim*vp1*delt/2.d0)
	cphase2 = cdexp(-cim*vp2*delt/2.d0)
	cw1(ll) = cw1(ll)*cphase1
	cw2(ll) = cw2(ll)*cphase2
	cwtemp=(cw1(ll)*b(1,1)+cw2(ll)*(b(1,2)))
	cw2(ll)=(cw2(ll)*b(2,2)+cw1(ll)*(b(1,2)))
	cw1(ll)=cwtemp
