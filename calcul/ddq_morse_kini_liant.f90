...
real(8) ::  wir,phase,tc,te,tf,delr,massreduite,dt,t0,kini
...
character*17 :: chinam
namelist /iofile/ t0, pulsetype,E0,phase,w,dt,tf,npos,v,kini

iE0=0
!WRITE(test,'(I5.5)') iE0
!write(*,*) "case_"//ADJUSTL(test)//"/input"
!open(5,name="case_"//ADJUSTL(test)//"/input",status="old")
open(5,file='input_kini',status='old')
read(5,nml=iofile)
...
delr=4.d-3
        ...
        do i=1,npos
         x(i)=1.89d-1+(i-1)*delr !création de l'axe des positions
        enddo
...
alpha = 13.019d0 !paramètre pour chi1 et chi2
 ...
 do n=1,v
    ...
    enddo
          ...
          ep(n,:)=ep(n,:)/dsqrt(norme) !normalisation des etats vibrationnels

 enddo
 call eval(chi1, chi2, delr, 1.89d-1, p0, rc0, alpha, npos)

open(13,file="out/wvfct.dat")
do n=1,npos
  if (dble(chi1(n)).lt.(1.d-150)) then
    chi1(n)=dcmplx(0.d0,0.d0)
  else
    chi1(n)=chi1(n)*dcos(x(n)*kini)
  end if
end do

!********************************************************************
!	    Creation du paquet d'onde initial 100% liant
!********************************************************************
	 lieprob = 0.d0
     do n = 1, v
	       do j = 1, npos
...
do i = 1,npos
chi1(i) = dcmplx(0.d0,0.d0)
  do n=1,v
    chi1(i) = chi1(i) + lieprobv(n)*ep(n,i)
  end do
end do

    

 call pot_spec(pot(1,:),pot(2,:),xmu12, npos,delr,1.89d-1) !construction des 2 potentiels de H2+ et du moment dipolaire

open(1002,file='out/pot.dat')
do i=1,npos
write(1002,*) 1.89d-1+(i-1)*delr,pot(1,i),pot(2,i),xmu12(i)
end do
 close(1002)
!***********************************************************************
...
!***********************************************************************
	do l=1,npos
	if (cdabs(chi1(l)).lt.(1.d-150)) then
		chi1(l)=dcmplx(0.d0,0.d0)
		worka(l)=0.d0
	else
...
!   write(*,*) "NORME De Depart : ", normedeb
 ...
 !open(unit=234579+iE0,name='chi2init.dat',status='replace')
   do l = 1, npos
         ...
         write(13,'(3(es30.15e3))') x(l), chi1(l)
!********************************************************************
 ...
 close(13)
    ...
    !***********************************************************************
    !           Normalisation de la fonction d'onde initiale			!JN
    !***********************************************************************
!    do l=1,npos
